import steamreviews
import pandas as pd
from tqdm import tqdm  # Importujemy bibliotekę do pasków postępu

# ID gry: 292030 (Wiedzmin3)
app_id = 292030
...
print("UWAGA: Pobieranie może potrwać. Obserwuj logi tekstowe poniżej:")

# Pobieramy dane oddzielnie dla kazdego jezyka i laczymy je
for lang in jezyki_do_pobrania:
    print(f"\n-> Pobieranie paczki dla jezyka: {lang.upper()}...")
    request_params = dict(language=lang)

    # verbose=True gwarantuje, że steamreviews będzie informować o postępie w konsoli
...
print(f"\nZebrano lacznie {len(wszystkie_recenzje)} recenzji.")
print("Przetwarzanie danych i generowanie pliku Excel...")

# Przygotowujemy liste, do ktorej trafia przetworzone dane
data_for_excel = []

# OWIJAMY słownik w funkcję tqdm() - to wygeneruje dynamiczny pasek postępu!
for review_id, review_data in tqdm(wszystkie_recenzje.items(), desc="Analiza i kategoryzacja", unit=" recenzji"):
...
file_name = "recenzje_steam_analiza4.xlsx"
